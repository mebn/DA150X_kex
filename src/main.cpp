...
std::ostream &operator<<(std::ostream &os, const Gene &gene) {
    os << "[" << gene.id_job << ", " << gene.id_step << ", " << gene.id_operation << "]";
    return os;
}
    ...
    for (int id_job = 0; id_job < total_jobs; id_job++) {
        ...
        for (int id_step = 0; id_step < number_steps; id_step++) {
            ...
            for (int id_operation = 0; id_operation < number_operations; id_operation++) {
                ...
                input_data[id_job].steps[id_step].candidates[id_operation].id_machine = id_machine - 1;
                input_data[id_job].steps[id_step].candidates[id_operation].processing_time = processing_time;
            }
...
void init_population_kernel(Gene *population, int population_size, int individual_len, Job *jobs, int total_jobs, int index) {
...
void pick_parents_kernel(int *parents, int *parent_candidates, int *scores, int population_size, int index) {
    int i = index;
...
void assignment_crossover(Gene *child, Gene *parent_a, Gene *parent_b, int individual_len, Job *jobs) {
...
void sequencing_crossover(Gene *child, Gene *parent_a, Gene *parent_b, int individual_len, Job *jobs) {
...
void assignment_mutation(Gene *individual, int individual_len, Job *jobs) {
    ...
    while (count--) {
        ...
        individual[mutation_point].id_machine = jobs[id_job].steps[id_step].candidates[id_operation].id_machine;
    }
...
void swapping_mutation(Gene *individual, int individual_len, Job *jobs) {
    ...
    while (count--) {
        ...
        if (individual[mutation_point].id_job != individual[mutation_point + 1].id_job)
            std::swap(individual[mutation_point], individual[mutation_point + 1]);
    }
}

void stage_1_breed_kernel(int *parents, Gene *population, Gene *new_population, int population_size, int individual_len, Job *jobs, int index) {
    int i = index;
    if (i < population_size * 8 / 10) {
        ...
        for (int s = 0; s < individual_len; s++)
            new_population[i * individual_len + s] = population[parents[i] * individual_len + s];

        swapping_mutation(&new_population[i * individual_len], individual_len, jobs);
    }
}

void stage_2_breed_kernel(int *parents, Gene *population, Gene *new_population, int population_size, int individual_len, Job *jobs, int index) {
    int i = index;
    if (i < population_size * 4 / 10) {
        ...
        if (i < population_size * 9 / 10)
            assignment_mutation(&new_population[i * individual_len], individual_len, jobs);
        else
            swapping_mutation(&new_population[i * individual_len], individual_len, jobs);
    }
}

void stage_1_evaluate_kernel(int *scores, Gene *population, int population_size, int individual_len, Job *jobs, int index) {
    int value;
    ...
    if (index < population_size) {
        ...
        for (int s = 0; s < individual_len; s++) {
            ...
            for (int id_operation = 0; id_operation < len; id_operation++) {
                int processing_time = jobs[id_job].steps[id_step].candidates[id_operation].processing_time;
                int id_machine = jobs[id_job].steps[id_step].candidates[id_operation].id_machine;
...
void stage_2_evaluate_kernel(int *scores, Gene *population, int population_size, int individual_len, Job *jobs, int index) {
    int value;
    int machines[MAX_MACHINES];
    int last_step_id_machine[MAX_JOBS];
...
int main(int argc, const char *argv[]) {
    ...
    for (int i = 0; i < POPULATION_SIZE; i++)
        init_population_kernel(pop_ptr, POPULATION_SIZE, INDIVIDUAL_LEN, jobs, total_jobs, i);

    for (int i = 0; i < POPULATION_SIZE; i++)
        stage_1_evaluate_kernel(scores_ptr, pop_ptr, POPULATION_SIZE, INDIVIDUAL_LEN, jobs, i);

    int stage_1 = 3000;
    while (stage_1--) {
        for (int i = 0; i < POPULATION_SIZE * SIZE_PARENT_POOL; i++)
            fill_rand_kernel(parent_candidates, POPULATION_SIZE * SIZE_PARENT_POOL, POPULATION_SIZE, i);

        for (int i = 0; i < POPULATION_SIZE; i++)
            pick_parents_kernel(parents, parent_candidates, scores_ptr, POPULATION_SIZE, i);

        for (int i = 0; i < POPULATION_SIZE; i++)
            stage_1_breed_kernel(parents, pop_ptr, new_pop_ptr, POPULATION_SIZE, INDIVIDUAL_LEN, jobs, i);

        std::copy(new_population.begin(), new_population.end(), population.begin());
    ...
    while (stage_2--) {
        ...
        for (int i = 0; i < POPULATION_SIZE; i++)
            stage_2_breed_kernel(parents, pop_ptr, new_pop_ptr, POPULATION_SIZE, INDIVIDUAL_LEN, jobs, i);

        std::copy(new_population.begin(), new_population.end(), population.begin());

        for (int i = 0; i < POPULATION_SIZE; i++)
            stage_2_evaluate_kernel(scores_ptr, pop_ptr, POPULATION_SIZE, INDIVIDUAL_LEN, jobs, i);
